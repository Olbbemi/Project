<?php
    ...
    if($check === false)
    {
        SystemLog(-1, "Select_Account", "{$index_db_select_query}");
    ...
    if($index_db_fetch_query === null)
    {
        if($parsing_data['accountno'] !== null)
            SystemLog(-1, "Select_Account", "Not Member: {$parsing_data['accountno']}");
        else
            SystemLog(-1, "Select_Account", "Not Member: {$parsing_data['email']}");
    ...
    if($check === false)
    {
        SystemLog(-1, "Select_Account", "{$info_db_select_query}");
    ...
    $shard_db_select_query = "SELECT `*` FROM `account` WHERE `accountno` = \"{$index_db_fetch_query['accountno']}\"";
    $check = DB_ReadQuery($shard_db_handle, $shard_db_select_query, $shard_db_query_result);
    if($check === false)
    {
        SystemLog(-1, "Select_Account", "{$shard_db_select_query}");
    ...
    if($shard_db_fetch_query === null)
    {
        ...
        if($check === false)
        {
            ...
            SystemLog(-1,"Select_Account", $error);
            DB_Disconnection($index_db_handle);
            exit;
        }

        if($parsing_data['accountno'] !== null)
            SystemLog(-1, "Select_Account", "Not Alloc Account Table: {$parsing_data['accountno']}");
        else
            SystemLog(-1, "Select_Account", "Not Alloc Account Table: {$parsing_data['email']}");
    ...
    for($i = 0; $i < $fetch_query_size; $i++)
        $result[$fetch_query_key[$i]] = $shard_db_fetch_query[$fetch_query_key[$i]];
