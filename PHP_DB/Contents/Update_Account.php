<?php
    ...
    if($check === false)
    {
        SystemLog(-1, "Update_Account", "{$index_db_select_query}");
    ...
    if($check === false)
    {
        SystemLog(-1, "Update_Account", "{$info_db_select_query}");
    ...
    if($check === false)
    {
        ...
        if($check === false)
        {
            ...
            SystemLog(-1, "Update_Account", $error);
            DB_Disconnection($index_db_handle);
            exit;
        }
    ...
    # Update 쿼리문 제작
    $loop_count = 0;
    $query_sentence = "UPDATE `account` SET ";
