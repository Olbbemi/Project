...
/**-----------------------------------------------------------------------------------------------------------
  ...
  *-----------------------------------------------------------------------------------------------------------*/
void C_NetServer::M_NetS_Initialize(ST_NET_SERVER& pa_parse_data)
{
	StringCchCopy(m_log_action, _countof(m_log_action), _TEXT("NetServer"));

	v_accept_count = 0; v_recv_tps = 0; v_send_tps = 0;	v_accept_tps = 0;
	...
	m_max_session_count = pa_parse_data.parse_max_session;

	m_packet_code = pa_parse_data.parse_packet_code;
	m_first_key = pa_parse_data.parse_packet_key1; m_second_key = pa_parse_data.parse_packet_key2;

	m_probable_index = new C_LFStack<WORD>;

	m_thread_handle = new HANDLE[pa_parse_data.parse_make_work + 1];
	m_session_list = new ST_EN_SESSION[pa_parse_data.parse_max_session];
	for (int i = 1; i < pa_parse_data.parse_max_session; i++)
	{
...
void C_NetServer::M_NetS_Stop()
{
	...
	_tprintf(_TEXT("NetServer Stop Success!!!\n"));
}
...
/**-------------------------------------
  ...
  *-------------------------------------*/
bool C_NetServer::M_MatchIOCPHandle(SOCKET pa_socket, ST_EN_SESSION* pa_session)
{
...
unsigned int C_NetServer::M_Accept()
{
	...
	while (1)
	{
		...
		m_session_list[lo_avail_index].v_session_info[0] = IS_ALIVE;
		m_session_count++;
		...
		VIR_OnClientJoin(m_session_list[lo_avail_index].session_id);
...
unsigned int C_NetServer::M_PacketProc()
{
	while (1)
	{
		...
		ST_EN_SESSION* lo_session = nullptr;
		...
		{
			bool lo_is_disconnectable = false;
			ULONG lo_count = lo_session->v_send_count;

			for (int i = 0; i < lo_count; i++)
			{
				if (lo_session->store_buffer[i]->m_send_and_disconnect == true)
					lo_is_disconnectable = true;
			...
			if(lo_is_disconnectable == true)
				M_Disconnect(lo_session->session_id);
		}
...
/**----------------------------------------------------------------------------------------------------------------------------------
  ...
  *----------------------------------------------------------------------------------------------------------------------------------*/
void C_NetServer::M_Encode(C_Serialize* pa_serialQ)
{
	pa_serialQ->m_encode_enable = false;
...
/**----------------------------------------------------------------------------
  ...
  *----------------------------------------------------------------------------*/
bool C_NetServer::M_Decode(C_Serialize* pa_serialQ)
{
	BYTE lo_mixture_key = m_first_key ^ m_second_key;
	BYTE lo_sum = 0;
...
/**---------------------------------------------------------------------------------------------------------
  ...
  *---------------------------------------------------------------------------------------------------------*/
bool C_NetServer::M_SessionAcquireLock(LONG64 pa_session_id, WORD& pa_index)
{
...
/**--------------------------------------------------------------------------------
  ...
  *--------------------------------------------------------------------------------*/
void C_NetServer::M_SessionAcquireUnlock(LONG64 pa_session_id, WORD pa_index)
{
...
void C_NetServer::M_SendPacket(LONG64 pa_session_id, C_Serialize* pa_packet)
{
	...
	lo_check = M_SessionAcquireLock(pa_session_id, lo_index);
	if (lo_check == false)
	{
	...
	if (pa_packet->m_encode_enable == true)
		M_Encode(pa_packet);
	...
	M_SessionAcquireUnlock(pa_session_id, lo_index);
}
...
/**---------------------------------------
  ...
  *---------------------------------------*/
bool C_NetServer::M_RecvPost(ST_EN_SESSION* pa_session)
{
	WSABUF lo_wsabuf[2];
	...
	if (lo_recv_value == SOCKET_ERROR)
	{
		...
		if (lo_error_check != WSA_IO_PENDING)
		{
			...
			else if (lo_interlock_value < 0)
			{
				ST_Log* lo_log = new ST_Log({ "IO Count is Nagative: " + to_string(lo_interlock_value) });
				VIR_OnError(__LINE__, m_log_action, E_LogState::system, lo_log);
			}
		}
	}
...
/**--------------------------------------------------------------
  ...
  * send_flag == TRUE 이면 이미 전송 중을 의미 [send 수행 1회 제한]
  *--------------------------------------------------------------*/
char C_NetServer::M_SendPost(ST_EN_SESSION* pa_session)
{
...
LONG C_NetServer::M_NetAcceptCount()
{
	return v_accept_count;
}

LONG C_NetServer::M_NetAcceptTPS()
{
	return v_accept_tps;
}

LONG C_NetServer::M_NetRecvTPS()
{
	return v_recv_tps;
}

LONG C_NetServer::M_NetSendTPS()
{
	return v_send_tps;
}

LONG64 C_NetServer::M_NetTotalAcceptCount()
{
	return m_total_accept_count;
}

LONG C_NetServer::M_NetLFStackAlloc()
{
	return m_probable_index->M_GetAllocCount();
}

LONG C_NetServer::M_NetLFStackRemain()
{
	return m_probable_index->M_GetUseCount();
}

LONG C_NetServer::M_SerializeAllocCount()
{
	return C_Serialize::S_TLSAllocCount();
}

LONG C_NetServer::M_SerializeUseChunk()
{
	return C_Serialize::S_TLSChunkCount();
}

LONG C_NetServer::M_SerializeUseNode()
{
