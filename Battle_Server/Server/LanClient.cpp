...
void LanClient::LanC_Initialize(LAN_CLIENT& ref_data)
{
	...
	m_client.m_session_info[1] = 0;
	m_client.m_session_info[0] = IS_NOT_ALIVE;
...
void LanClient::Connect()
{
	...
	if (check == SOCKET_ERROR)
	{
		...
		{
			LOG_DATA* log = new LOG_DATA({ "Connect Fail Error [Code: " + to_string(WSAGetLastError()) + "]" });
			OnError(__LINE__, _TEXT("LanClient"), LogState::error, log);
		}
	}
	...
	{
		if (write_set.fd_count == 1)
		{
			InterlockedIncrement(&m_client.m_session_info[1]);

			m_client.m_send_count = 0;
			m_client.recvQ = new RingBuffer;
			m_client.sendQ = new LFQueue<Serialize*>;
			m_client.m_send_flag = FALSE;
			m_client.m_session_info[0] = IS_ALIVE;

			MatchIOCPHandle(m_client.socket, &m_client);
			RecvPost(&m_client);
			OnConnectComplete();

			LONG interlock_value = InterlockedDecrement(&m_client.m_session_info[1]);
...
void LanClient::CreateIOCPHandle(HANDLE& ref_handle, int run_work_count)
{
...
unsigned int LanClient::PacketProc()
{
	while (1)
	{
		...
		if (transfered == 0 && session == nullptr && overlap == nullptr) // 쓰레드 종료 조건
		{
			Serialize::Terminate();
			PostQueuedCompletionStatus(m_iocp_handle, 0, 0, 0);
		...
		{
			ULONG count = session->m_send_count;
			...
			InterlockedIncrement(&m_send_tps);
...
{
	InterlockedIncrement(&m_client.m_session_info[1]);
	if (m_client.m_session_info[0] != IS_ALIVE)
		return false;
...
void LanClient::SendPacket(Serialize* packet)
{
	...
	if (m_client.m_send_flag == FALSE)
	{
		InterlockedIncrement(&m_client.m_session_info[1]);

		ZeroMemory(&m_client.processOver, sizeof(OVERLAPPED));
		PostQueuedCompletionStatus(m_iocp_handle, 0, (ULONG_PTR)&m_client, &m_client.processOver);
	}
...
char LanClient::SendPost(EN_CLIENT* session)
{
	...
	while (1)
	{
		...
		session->m_send_count = use_count;
...
void LanClient::Release()
{
	...
	if (InterlockedCompareExchange64((LONG64*)m_client.m_session_info, IS_NOT_ALIVE, IS_ALIVE) != IS_ALIVE)
		return;

	// 각 세션의 완료된 송신 정보를 처리하기 위한 버퍼에 남아있는 리소스 반환
	int count = m_client.m_send_count;
	for (int i = 0; i < count; i++)
	...
	if (sendQ_count != 0)
	{
		...
		{
			m_client.sendQ->Dequeue(m_client.store_buffer[i]);
			Serialize::Free(m_client.store_buffer[i]);
		}
	}
	...
	// 서버에서 먼저 종료했으므로 클라이언트는 지속적으로 새로운 서버에 연결을 시도하기 위한 준비
	MakeSocket(m_lan_data_store.ip, m_lan_data_store.port);
	m_is_connect_success = false;
}
