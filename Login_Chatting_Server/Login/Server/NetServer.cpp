...
/**-----------------------------------------------------------------------------------------------------------
  ...
  *-----------------------------------------------------------------------------------------------------------*/
void C_NetServer::M_NetStart(ST_NET_SERVER& pa_parse_data)
{
	...
	m_session_list = new ST_Session[pa_parse_data.parse_max_session];
...
unsigned int C_NetServer::M_PacketProc()
{
	while (1)
	{
		...
		else if (lo_overlap == &lo_session->sendOver)
		{
			...
			VIR_OnSend(lo_session->session_id);
