...
namespace Olbbemi
{
	...
	private:
		...
		ST_CON_CLIENT* m_client_list;

		C_LFStack<WORD> *m_probable_index;

		void M_CreateIOCPHandle(HANDLE& pa_handle);
		bool M_MatchIOCPHandle(SOCKET pa_socket, ST_CON_CLIENT* pa_session);

		unsigned int M_Accept();
		unsigned int M_PacketProc();

		bool M_RecvPost(ST_CON_CLIENT* pa_session);
		char M_SendPost(ST_CON_CLIENT* pa_session);
