...
void C_LanServer::M_LanS_Initialize(ST_LAN_SERVER& pa_parse_data)
{
	...
	m_client_list = new ST_CON_CLIENT[pa_parse_data.parse_max_client];
...
/**-------------------------------------
  ...
  *-------------------------------------*/
bool C_LanServer::M_MatchIOCPHandle(SOCKET pa_socket, ST_CON_CLIENT* pa_client)
{
...
unsigned int C_LanServer::M_Accept()
{
	...
	while (1)
	{
		...
		LONG interlock_value = InterlockedDecrement(&m_client_list[lo_avail_index].v_session_info[1]);
...
unsigned int C_LanServer::M_PacketProc()
{
	while (1)
	{
		...
		ST_CON_CLIENT* lo_client = nullptr;
...
/**---------------------------------------
  ...
  *---------------------------------------*/
bool C_LanServer::M_RecvPost(ST_CON_CLIENT* pa_client)
{
	WSABUF lo_wsabuf[2];
...
/**---------------------------------------
  ...
  *---------------------------------------*/
char C_LanServer::M_SendPost(ST_CON_CLIENT* pa_client)
{
