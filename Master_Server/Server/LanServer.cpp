...
void LanServer::LanS_Initialize(LAN_SERVER& ref_data)
{
	...
	m_client_list = new CON_CLIENT[ref_data.max_client];
...
void LanServer::CreateIOCPHandle(HANDLE& ref_handle, int run_worker_thread)
{
	ref_handle = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, run_worker_thread);
}

bool LanServer::MatchIOCPHandle(SOCKET socket, CON_CLIENT* client)
{
...
unsigned int LanServer::Accept()
{
	...
	int error_check, keepalive_optval = 1, len = sizeof(server_address);
	...
	// 소켓 옵션 설정
	setsockopt(m_listen_socket, SOL_SOCKET, SO_KEEPALIVE, (char*)&keepalive_optval, sizeof(keepalive_optval));
	...
	while (1)
	{
		...
		{
			LOG_DATA* log = new LOG_DATA({ "Lan_Connect Limit Over [MaxClient: " + to_string(m_max_client_count) + "]" });
		...
		{
			LOG_DATA* log = new LOG_DATA({ "IO Count is Nagative [Alloc Index: " + to_string(m_client_list[avail_index].alloc_index) + ", Value: " + to_string(interlock_value) + "]" });
			OnError(__LINE__, _TEXT("LanServer"), LogState::system, log);
		}
	}
	...
	while (1)
	{
		...
		else if (transfered != 0 && overlap == &client->recvOver) // 수신된 패킷 처리
		{
			...
			while (1)
			{
				...
				recv_size = client->recvQ->GetUseSize();
		...
		if (recvpost_value == true && sendpost_value != -1)
		{
			...
			{
				LOG_DATA* log = new LOG_DATA({ "IO Count is Nagative [Alloc Index: " + to_string(client->alloc_index) + ", Value: " + to_string(interlock_value) + "]" });
				OnError(__LINE__, _TEXT("LanServer"), LogState::system, log);
			}
		}
	}
...
void LanServer::SessionAcquireUnlock(WORD index)
{
	...
	{
		LOG_DATA* log = new LOG_DATA({ "IO Count is Nagative [Alloc Index: " + to_string(m_client_list[index].alloc_index) + ", Value: " + to_string(interlock_value) + "]" });
		OnError(__LINE__, _TEXT("LanServer"), LogState::system, log);
	}
}
...
void LanServer::SendPacket(WORD index, Serialize* packet)
{
	...
	SendPost(&m_client_list[index]);
	Serialize::Free(packet);

	SessionAcquireUnlock(index);
}

bool LanServer::RecvPost(CON_CLIENT* client)
{
	WSABUF wsabuf[2];
	DWORD flag = 0;
	DWORD size = 0;
...
char LanServer::SendPost(CON_CLIENT* client)
{
...
void LanServer::Release(WORD index)
{
	...
	m_client_list[index].recvQ = nullptr;

	delete m_client_list[index].sendQ;
	m_client_list[index].sendQ = nullptr;
...
LONG LanServer::SerializeAllocCount()
{
	return Serialize::TLSAllocCount();
}

LONG LanServer::SerializeUseChunk()
{
	return Serialize::TLSChunkCount();
}

LONG LanServer::SerializeUseNode()
{
