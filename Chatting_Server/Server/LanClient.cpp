...
void LanClient::CreateIOCPHandle(HANDLE& ref_handle, int run_work_thread)
{
...
void LanClient::Release()
{
	...
	m_client.recvQ = nullptr;

	delete m_client.sendQ;
	m_client.sendQ = nullptr;

	// 배틀서버가 종료되면 채팅서버도 같이 종료되는 구조 때문에 호출하는 함수
	OnDisconnect();
