...
{
	StringCchCopy(m_log_action, _countof(m_log_action), _TEXT("ChatNetServer"));

	m_total_accept = 0;	v_accept_count = 0;
	v_accept_tps = 0;	v_recv_tps = 0;	v_send_tps = 0;
	...
	while (1)
	{
		...
		else if (lo_transfered != 0 && lo_overlap == &lo_session->recvOver)
		{
			...
			while (1)
			{
				...
				if (lo_header.len >= 1024)
				{
					lo_check = false;
					M_Release(lo_session->session_id);
					break;
				}
				else if (lo_recv_size < lo_header.len + NET_HEAD_SIZE)
					break;					
