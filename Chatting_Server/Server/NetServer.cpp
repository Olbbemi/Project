...
void NetServer::NetS_Initialize(NET_SERVER& ref_data)
{
	// 모니터링 변수 초기화
	m_total_accept = 0;	
	m_accept_count = 0;
	...
	for (int i = 0; i < ref_data.max_session; i++)
	{
		m_session_list[i].m_session_info[1] = 0;
		m_session_list[i].m_session_info[0] = IS_NOT_ALIVE;
...
void NetServer::NetS_Stop()
{
	...
	{
		if (m_session_list[i].m_session_info[0] == IS_ALIVE)
		{
	...
	while (1)
	{
		...
		for (int i = 0; i < m_max_session_count; i++)
		{
			if (m_session_list[i].m_session_info[1] != 0)
			{
				flag = true;
				break;
			}
	...
	{
		LOG_DATA* log = new LOG_DATA({ "WaitForMultipleObjects Error [Code: " + to_string(WSAGetLastError()) + "]" });
		OnError(__LINE__, _TEXT("NetServer"), LogState::system, log);
	}
...
void NetServer::CreateIOCPHandle(HANDLE& ref_handle, int run_work_thread)
{
	ref_handle = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, run_work_thread);
}
...
unsigned int NetServer::Accept()
{
	...
	while (1)
	{
		...
		InterlockedIncrement(&m_session_list[avail_index].m_session_info[1]);
		
		m_session_list[avail_index].active_cancle_io = false;
		...
		m_session_list[avail_index].m_send_flag = FALSE;			
		m_session_list[avail_index].m_session_info[0] = IS_ALIVE;
		m_session_count++;
		...
		LONG interlock_value = InterlockedDecrement(&m_session_list[avail_index].m_session_info[1]);
...
unsigned int NetServer::PacketProc()
{
	while (1)
	{
		...
		bool gqcs_check = GetQueuedCompletionStatus(m_iocp_handle, &transfered, (ULONG_PTR*)&session, &overlap, INFINITE);
		if (gqcs_check == false)
		{
			int error_check = GetLastError();
		...
		else if (session->active_cancle_io == false && overlap == &session->sendOver) // 완료된 송신 패킷 처리
		{
			...
			InterlockedBitTestAndReset(&session->m_send_flag, 0);
		...
		else if (overlap == &session->processOver) // 각 세션에 존재하는 패킷 전송
			SendPost(session);

		// RecvPost, SendPost, SendProcess 에서 해당 세션의 IO_Count 를 차감하지 않았다면 1회 차감
		if (recvpost_value == true && sendpost_value != -1)
		{
			LONG interlock_value = InterlockedDecrement(&session->m_session_info[1]);
			if (interlock_value == 0)
				Release(session->session_id);
			else if (interlock_value < 0)
			{
...
bool NetServer::SessionAcquireLock(LONG64 session_id, WORD& ref_index)
{
	...
	InterlockedIncrement(&m_session_list[index].m_session_info[1]);
	if (m_session_list[index].m_session_info[0] != IS_ALIVE || m_session_list[index].session_id != session_id)
		return false;
...
{
	LONG interlock_value = InterlockedDecrement(&m_session_list[index].m_session_info[1]);
	if (interlock_value == 0)
		Release(session_id);
	else if (interlock_value < 0)
	{
		LOG_DATA* log = new LOG_DATA({ "IO Count is Nagative [SessionId: " + to_string(session_id) + ", Value: " + to_string(interlock_value) + "]" });
		OnError(__LINE__, _TEXT("NetServer"), LogState::system, log);
	}
}
...
void NetServer::SendPacket(LONG64 session_id, Serialize* packet)
{
	...
	if (m_session_list[index].m_send_flag == FALSE)
	{
		InterlockedIncrement(&m_session_list[index].m_session_info[1]);
		
		ZeroMemory(&m_session_list[index].processOver, sizeof(OVERLAPPED));
		PostQueuedCompletionStatus(m_iocp_handle, 0, (ULONG_PTR)&m_session_list[index], &m_session_list[index].processOver);
	}
...
bool NetServer::RecvPost(EN_SESSION* session)
{
	...
	InterlockedIncrement(&session->m_session_info[1]);

	int wsarecv_value = WSARecv(session->socket, wsabuf, buffer_count, &size, &flag, &session->recvOver, NULL);
...
char NetServer::SendPost(EN_SESSION* session)
{
	...
	{
		// 각 세션마다 Send 1회 제한
		if (InterlockedCompareExchange(&session->m_send_flag, TRUE, FALSE) == TRUE)
			return 1;
		...
		for (int i = 0; i < use_count; i++)
		{
			...
			wsabuf[buffer_count].buf = session->store_buffer[i]->GetBufferPtr();
			wsabuf[buffer_count++].len = session->store_buffer[i]->GetUsingSize();
		...
		int wsasend_value = WSASend(session->socket, wsabuf, buffer_count, &size, 0, &session->sendOver, NULL);
...
void NetServer::Release(LONG64 session_id)
{
	...
	if (InterlockedCompareExchange64((LONG64*)m_session_list[index].m_session_info, IS_NOT_ALIVE, IS_ALIVE) != IS_ALIVE)
		return;
	...
	m_session_list[index].recvQ = nullptr;

	delete m_session_list[index].sendQ;
	m_session_list[index].sendQ = nullptr;

	closesocket(m_session_list[index].socket);
	m_session_list[index].m_session_info[0] = false;
